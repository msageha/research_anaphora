...
def predict(model_path, test_data, domain, case, args):
    ...
    with open('{0}/args/domain-{1}_case-{2}.json'.format(args.dir, 'OC', case)) as f:
        tmp = json.load(f)
        for key in tmp.keys():
            if key == 'dir':
                continue
    ...
    for case in ['ga', 'o', 'ni']:
        for model_path in load_model_path(args.dir, case):
            ...
            for domain in domain_dict:
                ...
                test_data  = tuple_dataset.TupleDataset(test_x, test_y, test_y_dep_tag, test_word, test_is_verb)
