...
def training(train_dataset_dict, test_dataset_dict, type_statistics_dict, domain, case, dump_path, args):
...
def union(dataset_dict, args, dump_path):
    ...
    for domain in domain_dict:
        ...
        dev_dataset_dict['{0}_x'.format(domain)] = np.array(dataset_dict['{0}_x'.format(domain)][train_size:dev_size])
        train_dataset_dict['{0}_y_ga'.format(domain)] = np.array(dataset_dict['{0}_y_ga'.format(domain)][:train_size])
        dev_dataset_dict['{0}_y_ga'.format(domain)] = np.array(dataset_dict['{0}_y_ga'.format(domain)][train_size:dev_size])
        train_dataset_dict['{0}_y_o'.format(domain)] = np.array(dataset_dict['{0}_y_o'.format(domain)][:train_size])
        dev_dataset_dict['{0}_y_o'.format(domain)] = np.array(dataset_dict['{0}_y_o'.format(domain)][train_size:dev_size])
        train_dataset_dict['{0}_y_ni'.format(domain)] = np.array(dataset_dict['{0}_y_ni'.format(domain)][:train_size])
        dev_dataset_dict['{0}_y_ni'.format(domain)] = np.array(dataset_dict['{0}_y_ni'.format(domain)][train_size:dev_size])
...
def fine_tuning(model_path, train_dataset_dict, test_dataset_dict, type_statistics_dict, domain, case, dump_path, args):
    ...
    with open('{0}/args/domain-{1}_case-{2}.json'.format(dump_path, domain, case), 'w') as f:
        ...
        args.__dict__['fine_tuning_test_size'] = test_data_size
        json.dump(args.__dict__, f, indent=2)
    print(json.dumps(args.__dict__, indent=2))
    ...
    for epoch in range(1, args.epoch+1):
        ...
        if test_total_accuracy > max_accuracy:
            model.to_cpu()
            chainer.serializers.save_npz("{0}/fine_tuning/domain-{1}_case-{2}_epoch-{3}.npz".format(dump_path, domain, case, epoch), model)
            model.to_gpu()
        max_accuracy = max(max_accuracy, test_total_accuracy)
        logs.append({
            "main/loss": float(train_total_loss),
        ...
        print('{0}\t{1}\t{2}\t{3}\t{4}\t{5}'.format(epoch, train_total_loss, train_total_accuracy, test_total_loss, test_total_accuracy, time.time() - st))
    with open('{0}/log/domain-{1}_case-{2}_fine_tuning.json'.format(dump_path, domain, case), 'w') as f:
        json.dump(logs, f, indent=4)
